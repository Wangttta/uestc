...
class MUMEC:
    ...
    def __init__(self, args):
        ...
        # 2.3. 离散动作空间：每个无人机的二维移动，每个维度是一个属于 {-1, 0, 1} 的离散变量
            ...
            self.action_space = MultiDiscrete([9] * self.N)
        ...
        self.dis_mat, self.matches = self.__init_matching()
        self.step_count = 0
    ...
    def __init_matching(self):
        distance_matrix = np.linalg.norm(self.uav_pos[:, np.newaxis] - self.ue_pos, axis=2)
        matches = []
        for n in range(self.N):
            sorted_indices = np.argsort(distance_matrix[n])
            for m in sorted_indices:
                if m not in matches:
                    matches.append(m)
                    break
        return distance_matrix, np.array(matches, dtype=int)
    ...
    def step_action(self, action):
        ...
        # 2. Execute action
        x = np.array((action // 3 - 1) * 10, dtype=np.float32)
        y = np.array((action % 3 - 1) * 10, dtype=np.float32)
        z = np.zeros(self.N, dtype=np.float32)
        self.var_w = np.column_stack((x, y, z))  # UAVs movement
    ...
    def __calc_reward(self):
        distance = np.linalg.norm(self.uav_pos - self.ue_pos[self.matches], axis=-1)
        distance_init = self.dis_mat[np.arange(len(self.dis_mat)), self.matches]
        reward = np.sum(- distance / distance_init) / self.N
        done = np.all(distance <= 100)
        return reward, done
